      ...
      <Card className={`w-full max-w-sm shadow-2xl border ${cardBg}`}>
        <CardHeader>
          ...
          <CardDescription className={descCls}>
            A belépéshez adja meg e-mail-címét.
          </CardDescription>
          <CardAction>
        ...
        <form onSubmit={handleSubmit}>
          <CardContent className="grid gap-6">
            <div className="grid gap-2 text-left">
              <Label htmlFor="email" className={`font-semibold ${labelCls}`}>E-mail cím</Label>
              <Input
                id="email" name="email" type="email" placeholder="pelda@gmail.com"
            ...
            <div className="grid gap-2 text-left">
              ...
              <div className="relative">
                <Input
                  ...
                  className={`h-11 focus:ring-2 pr-10 mb-2 ${inputCls}`}
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(v => !v)}
                  className={`absolute right-3 top-3 transition-colors ${eyeCls}`}
                  tabIndex={-1}
