    ...
    <nav className={`sticky top-0 z-50 backdrop-blur-md border-b transition-colors duration-300 ${navBg}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-8 flex justify-between items-center h-16">
        ...
        <div className="hidden lg:flex items-center gap-3">
          <button
            onClick={toggleTheme}
            className={`w-9 cursor-pointer h-9 rounded-lg flex items-center justify-center transition-colors ${
      ...
      {/* Mobil menü */}
      <div className={`lg:hidden overflow-hidden transition-all duration-300 ease-in-out ${menuOpen ? "max-h-48" : "max-h-0"}`}>
        <div className={`border-t px-4 pb-4 pt-2 space-y-2 ${mobileBg}`}>
          <button
            onClick={toggleTheme}
            className={`w-full flex cursor-pointer items-center gap-2 py-2.5 px-3 rounded-lg text-sm transition-colors ${
